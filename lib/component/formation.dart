...
import 'package:hair_app/model/player.dart';
...
class _FormationState extends ConsumerState<Formation> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(formationProvider.notifier).initializeFormation(widget.matchDate, widget.opponent);
  ...
  void _initializeFormation() {
    ...
    for (int i = 0; i < initialPositions.length; i++) {
      notifier.updatePlayerPosition(widget.matchDate, widget.opponent, i, initialPositions[i]);
    }
  }
  ...
  void _updatePosition(int index, Offset newPosition) {
    ...
    ref.read(formationProvider.notifier).updatePlayerPosition(
      widget.matchDate,
      widget.opponent,
      index,
  ...
  void _editPlayer(int index) {
    // プレイヤーの名前編集等は各自実装（ここでは省略）
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          title: const Text('選手を編集'),
          content: const Text('この機能は必要に応じて実装してください。'),
          actions: [
  ...
  Future<bool> _onWillPop() async {
    ...
    if (save == true) {
      ...
      if (currentData != null) {
        ...
        ref.read(formationProvider.notifier).updateFormation(widget.matchDate, widget.opponent, newData);
      }
      print('Changes saved.');
    } else {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text(
            'フォーメーション - ${widget.opponent} (${widget.matchDate})',
  ...
  Widget _buildScoreBoard(FormationData formationData) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          _buildScoreColumn("自チーム", formationData.myScore, () {
            ref.read(formationProvider.notifier).updateMyScore(widget.matchDate, widget.opponent, formationData.myScore + 1);
          ...
          _buildScoreColumn("相手チーム", formationData.opponentScore, () {
            ref.read(formationProvider.notifier).updateOpponentScore(widget.matchDate, widget.opponent, formationData.opponentScore + 1);
            setState(() { _hasChanges = true; });
          }, () {
  ...
  Widget _buildScoreColumn(String teamName, int score, VoidCallback onIncrease, VoidCallback onDecrease) {
    return Column(
      children: [
        ...
        Row(
          children: [
            IconButton(onPressed: onIncrease, icon: const Icon(Icons.arrow_drop_up, size: 40, color: Colors.green)),
            Text('$score', style: const TextStyle(fontSize: 30, fontWeight: FontWeight.bold)),
            IconButton(onPressed: onDecrease, icon: const Icon(Icons.arrow_drop_down, size: 40, color: Colors.red)),
          ],
        ),
      ],
    );
  ...
  Widget _buildPlayer(int index, {bool isDragging = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.red.withOpacity(isDragging ? 0.5 : 1.0),
